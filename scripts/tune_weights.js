...
function makeEval(w){
  return function(grid){
    ...
    for(let r=0;r<4;r++)for(let c=0;c<4;c++){
      const v=grid[r][c];if(v===0)continue;const lv=Math.log2(v);
    ...
    let merges=0;
    for(let r=0;r<4;r++)for(let c=0;c<4;c++){const v=grid[r][c];if(c<3&&v===grid[r][c+1]&&v>0)merges++;if(r<3&&v===grid[r+1][c]&&v>0)merges++;}
    let corner=0;
    if(max>0)for(const[r,c]of[[0,0],[0,3],[3,0],[3,3]])if(grid[r][c]===max){corner=max*w.corner;break;}
    const emptyBonus=empty===0?0:Math.pow(empty,1.5)*w.empty;
    const maxBonus=Math.log2(max||1)*w.maxTile;
    return structure*w.structure+emptyBonus+corner+smoothness*w.smoothness+merges*w.merges+maxBonus;
  };
}

function exAB(grid,depth,isChance,alpha,beta,w){
  ...
  if(!isChance){
    ...
    for(const m of moves){
      const cv=m.gained+exAB(m.board,depth-1,true,alpha,beta,w);
    ...
    if(!cells.length)return exAB(grid,depth-1,false,alpha,beta,w);
    ...
    for(let i=0;i<n;i++){const[r,c]=cells[i*step];const t2=clone(grid);t2[r][c]=2;const t4=clone(grid);t4[r][c]=4;
      total+=0.9*exAB(t2,depth-1,false,alpha,beta,w)+0.1*exAB(t4,depth-1,false,alpha,beta,w);}
    value=total/n;
  }
  return value;
}

function bestMove(grid,depth,w){
  ...
  for(const m of moves){const v=m.r.gained+exAB(m.r.board,depth,true,-Infinity,Infinity,w);if(v>bestScore){bestScore=v;bestDir=m.d;}}
  return bestDir;
}
...
function playGame(seed,depth,w){
  ...
  while(hasMoves(b)&&m<3000){
    const d=bestMove(b,depth,w);if(d===null)break;
    const{board:nb}=moveFast(b,d);b[0]=nb[0];b[1]=nb[1];b[2]=nb[2];b[3]=nb[3];addTile(b,r);m++;
  }
  const mx=Math.max(...b.flat());return{win:mx>=2048,maxTile:mx};
}

function testConfig(w,depth,N){
  let wins=0;
  for(let i=0;i<N;i++)if(playGame(800000+i,depth,w).win)wins++;
  return wins;
}

// Quick search with smaller N
const N=10;const depth=3;
console.log('=== Weight Tuning (depth='+depth+', N='+N+') ===\n');

const configs=[
  // empty × corner grid
  {name:'e80_c40',w:{structure:0.5,empty:80,corner:40,smoothness:10,merges:30,maxTile:40}},
  {name:'e100_c40',w:{structure:0.5,empty:100,corner:40,smoothness:10,merges:30,maxTile:40}},
  {name:'e120_c40',w:{structure:0.5,empty:120,corner:40,smoothness:10,merges:30,maxTile:40}},
  {name:'e100_c60',w:{structure:0.5,empty:100,corner:60,smoothness:10,merges:30,maxTile:40}},
  {name:'e120_c60',w:{structure:0.5,empty:120,corner:60,smoothness:10,merges:30,maxTile:40}},
  {name:'e100_c80',w:{structure:0.5,empty:100,corner:80,smoothness:10,merges:30,maxTile:40}},
  {name:'e150_c60',w:{structure:0.5,empty:150,corner:60,smoothness:10,merges:30,maxTile:40}},
  {name:'e100_c100',w:{structure:0.5,empty:100,corner:100,smoothness:10,merges:30,maxTile:40}},
  {name:'e150_c100',w:{structure:0.5,empty:150,corner:100,smoothness:10,merges:30,maxTile:40}},
  // Add structure variations
  {name:'e100_c60_s0.8',w:{structure:0.8,empty:100,corner:60,smoothness:10,merges:30,maxTile:40}},
  {name:'e100_c60_s0.3',w:{structure:0.3,empty:100,corner:60,smoothness:10,merges:30,maxTile:40}},
  {name:'e100_c60_m50',w:{structure:0.5,empty:100,corner:60,smoothness:10,merges:50,maxTile:40}},
  {name:'e100_c60_mt60',w:{structure:0.5,empty:100,corner:60,smoothness:10,merges:30,maxTile:60}},
];

const results=[];
for(const c of configs){
  const w=makeEval(c.w);
  const wins=testConfig(w,depth,N);
  results.push({name:c.name,wins,N,winRate:wins/N*100,w:c.w});
  console.log(c.name.padEnd(15)+': '+(wins/N*100).toFixed(0)+'% win ('+wins+'/'+N+')');
}

results.sort((a,b)=>b.winRate-a.winRate);
console.log('\n=== Top 3 ===');
results.slice(0,3).forEach((r,i)=>console.log((i+1)+'. '+r.name+': '+r.winRate.toFixed(0)+'% | '+JSON.stringify(r.w)));
